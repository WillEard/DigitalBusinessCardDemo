...
export const register = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: REGISTER_TEMPLATE.replace("{{email}}", email).replace(
        "{{user}}",
        user.username
      ),
    };
...
export const login = async (req, res) => {
  ...
  try {
    ...
    res.cookie("token", token, {
      ...
      maxAge: 7 * 24 * 60 * 60 * 1000,
      domain:
        process.env.NODE_ENV === "production" ? ".pelagopass.com" : undefined,
    });
...
export const sendVerifyOtp = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Email OTP verification has been sent.",
    });
  } catch (error) {
...
export const sendResetOTP = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: PASSWIRD_RESET_TEMPLATE.replace("{{otp}}", otp)
        .replace("{{email}}", user.email)
        .replace("{{name}}", user.name),
    };
...
export const verifyReset = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ valid: false, message: "Error verifying OTP" });
  }
};
