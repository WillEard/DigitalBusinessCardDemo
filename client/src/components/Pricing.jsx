...
const PricingContainer = () => {
  ...
  const handlePaymentPage = useCallback(
    async (e) => {
      ...
      <Row className="justify-content-center">
        {plans.map(({ id, title, features, buttonText, buttonClass }) => (
          <Col key={id} md={id === "free" ? 4 : 6} className="mb-4">
            <Card
              className={`h-100 shadow-sm ${id}-plan text-center rounded-5`}
            >
              ...
              <Card.Body className="plan-content mb-3">
                <ul className="list-unstyled mt-3 fontCondensed">
                  {features.map(({ text, included }, i) => (
                    <li key={i}>
                      {included ? (
                        <FaCheck className="icon-inline" aria-hidden="true" />
                      ) : (
                        <RxCross2 className="icon-inline" aria-hidden="true" />
                      )}
                      {text}
                    </li>
                  ))}
